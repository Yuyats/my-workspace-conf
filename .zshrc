...
alias f='git fetch'
alias cm='git commit'
alias log='git log --graph --all --format="%x09 %C(red)%ad %C(yellow)%h %C(cyan bold)%<(20,trunc)%an %x09 %C(reset)%C(blue reverse)%d%C(reset) %C(blue)%s" --date=format:"[%Y-%m-%d %H:%M:%S]"'
alias b='git branch'
alias ..='cd ..'
alias cl='clear'
alias py='python'
